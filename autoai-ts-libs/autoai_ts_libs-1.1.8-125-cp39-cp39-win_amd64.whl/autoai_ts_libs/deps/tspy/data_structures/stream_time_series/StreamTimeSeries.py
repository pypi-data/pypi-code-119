...
class StreamTimeSeries:
    ...
    def add_sink(self, data_sink):
        ...
        >>> from autoai_ts_libs.deps.tspy.io.DataSink import DataSink
        ...
        return StreamTimeSeries(
            self._tsc,
            self._j_stream_time_series.addSink(utils.JavaDataSink(self._tsc, data_sink))
        )
    ...
    def map(self, func):
        ...
        """
        return StreamTimeSeries(self._tsc, self._j_stream_time_series.map(utils.UnaryMapFunction(self._tsc, func)), self._trs)
    ...
    def filter(self, func):
        ...
        """
        return StreamTimeSeries(self._tsc, self._j_stream_time_series.filter(utils.FilterFunction(self._tsc, func)), self._trs)
    ...
    def flatmap(self, func):
        ...
        return StreamTimeSeries(self._tsc, self._j_stream_time_series.flatMap(
            utils.FlatUnaryMapFunction(self._tsc, func)), self._trs)
    ...
    def segment_by_time(self, window, step=None):
        ...
        return SegmentStreamTimeSeries(
            self._tsc,
            self._j_stream_time_series.segmentByTime(window, step),
            self._trs
        )
    ...
    def segment_by_anchor(self, anchor_op, left_delta, right_delta):
        ...
        return SegmentStreamTimeSeries(
            self._tsc,
            self._j_stream_time_series.segmentByAnchor(utils.FilterFunction(self._tsc, anchor_op), left_delta, right_delta),
            self._trs
        )
