...
class DiscriminatorySubSequenceModel:
    ...
    def __getstate__(self):
        j_model_str = self._tsc._jvm.com.ibm.research.time_series.ml.sequence_mining.PythonConnector.serializeDiscriminatorySubSequenceModel(self._j_model)
        return {'j_model': j_model_str}

    def __setstate__(self, d):
        from autoai_ts_libs.deps.tspy.data_structures.context import get_or_create
        self._tsc = get_or_create()
        self._j_model = self._tsc._jvm.com.ibm.research.time_series.ml.sequence_mining.PythonConnector.deserializeDiscriminatorySubSequenceModel(d['j_model'])
        dss_list = []
        for dss in self._j_model.discriminatorySubSequences():
            dss_list.append(DiscriminatorySubSequence(dss))
        self._dss_list = dss_list
